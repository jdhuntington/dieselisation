...
MapManager = {
  ...
  tilePosition: function(row, column)
  {
    var rowIdx = $A(board['rows']).indexOf(row);
    var actualColumn = Math.floor((column - 1) / 2);
    var offset = (rowIdx % 2 == 1) ? M.width / 2 : 0;

    return { position: 'absolute',
	     top: '' + Math.round(rowIdx * M.bv) +'px',
	     left: '' + (Math.round(actualColumn * M.width) + offset) +'px'
	   };
  },
  addTile: function(layRotation, cell, row, column)
  {
    ...
    tile.setStyle(this.tilePosition(row, column));
    this.drawTile(tile, cell, layRotation);
    this.map.appendChild(tile);
    this.tiles[this.tiles.length] = tile;
  },
  drawTile: function(tileNode, cell, layRotation)
  {
    var ctx = tileNode.getContext("2d");
    var color = 'empty';
    if(cell['endpoint'] && cell['endpoint'] == 'yes') color = 'red';
    ...
    ctx.save();
    if(cell['city']) MapManager.drawStation(ctx);
//     if(tileData.segments)
//       {
...
//       }
  ...
  drawStation: function(ctx) {
    ctx.beginPath();
    ctx.fillStyle = '#ffffff';
    ctx.strokeStyle = '#222222';
    ctx.lineWidth = 3;
    ctx.arc(0, 0, M.h / 1.5, 0, Math.PI * 2, true);
    ctx.stroke();
    ctx.fill();
    ctx.restore();
  },
...
Dataset = {
  load: function()
  {
    $A(board['rows']).each(function(row) {
